...
export default function ChatPage() {
  // Reset thread khi vào lại trang
  useEffect(() => {
    // Clear old thread khi vào lại trang để AI chào lại
    const shouldReset = sessionStorage.getItem('chat_should_reset')
    if (shouldReset === 'true' || !sessionStorage.getItem('chat_thread_id')) {
      sessionStorage.removeItem('chat_thread_id')
      sessionStorage.removeItem('chat_should_reset')
    }
  }, [])
  ...
  const handleSend = async () => {
    ...
    try {
      // Nếu đây là message đầu tiên sau greeting, tạo thread mới để AI chào lại
      // Nếu đã có conversation, tiếp tục thread cũ
      const isFirstUserMessage = messages.length === 1 // Chỉ có greeting message
      
      let threadId: string
      if (isFirstUserMessage) {
        // Tạo thread mới cho conversation mới
        threadId = `thread_${Date.now()}`
        sessionStorage.setItem('chat_thread_id', threadId)
      } else {
        // Dùng thread hiện tại
        threadId = sessionStorage.getItem('chat_thread_id') || `thread_${Date.now()}`
        if (!sessionStorage.getItem('chat_thread_id')) {
          sessionStorage.setItem('chat_thread_id', threadId)
        }
      }

      // Prepare chat history (bỏ qua greeting message đầu tiên)
      ...
      // Get user ID from Supabase - thử cả getUser() và getSession()
      let userId: string | null = null
      try {
        const supabase = createClient()
        
        // Thử getSession() trước (nhanh hơn)
        const { data: { session }, error: sessionError } = await supabase.auth.getSession()
        if (session?.user) {
          userId = session.user.id
          console.log("✅ User ID from session:", userId)
        } else {
          // Nếu không có session, thử getUser()
          const { data: { user }, error: userError } = await supabase.auth.getUser()
          if (userError) {
            console.warn("Error getting user:", userError)
          } else if (user) {
            userId = user.id
            console.log("✅ User ID from getUser():", userId)
          } else {
      ...
      if (!userId) {
        console.warn("⚠️ No user_id available - AI may not be able to filter tasks correctly")
        console.warn("⚠️ Please make sure you are logged in before using the chat")
      } else {
        console.log("✅ Sending user_id to AI service:", userId)
      }
  ...
  const getAIResponseFallback = (userInput: string): string => {
